...
import { Plus, List, Loader, Edit, Trash, Filter, ArrowUp, ArrowDown } from 'lucide-react';
...
const SortableHeader = ({ children, name, sortConfig, onSort }) => {
  ...
  return (
    <th
      scope="col"
      className="px-3 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-600/50 transition-colors"
      onClick={() => onSort(name)}
    >
...
const InventoryList = ({ inventory, customers, onSave, onDelete, loading, error }) => {  const [view, setView] = useState('active'); // 'active' or 'archived'
  ...
  const [isFormModalOpen, setIsFormModalOpen] = useState(false); // New state for form modal
          ...
          <div className="overflow-x-auto bg-gray-800 rounded-xl shadow-lg">
            <table className="min-w-full divide-y divide-gray-700">
              <thead className="bg-gray-700/50">
                <tr>
                  <SortableHeader name="stockNumber" sortConfig={sortConfig} onSort={requestSort}><div className="w-28">Stock Number</div></SortableHeader>
                  <SortableHeader name="inboundOrderNumber" sortConfig={sortConfig} onSort={requestSort}><div className="w-28">Order Number</div></SortableHeader>
                  <SortableHeader name="description" sortConfig={sortConfig} onSort={requestSort}><div className="w-48">Description</div></SortableHeader>
                  <SortableHeader name="quantity" sortConfig={sortConfig} onSort={requestSort}><div className="w-12">Qty</div></SortableHeader>
                  <SortableHeader name="location" sortConfig={sortConfig} onSort={requestSort}><div className="w-24">Stock Location</div></SortableHeader>
                  <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider"><div className="w-24">Status</div></th>
                  <SortableHeader name="inboundDate" sortConfig={sortConfig} onSort={requestSort}><div className="w-24">Inbound Date</div></SortableHeader>
                  <SortableHeader name="inboundReference" sortConfig={sortConfig} onSort={requestSort}><div className="w-28">Reference</div></SortableHeader>
                  <th className="px-3 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider"><div className="w-24">Actions</div></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-700">
                {Array.isArray(filteredInventory) && filteredInventory.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-700/50 transition-colors cursor-pointer" onClick={() => handleViewDetails(item)}>
                    <td className="px-3 py-4 whitespace-nowrap text-sm font-medium text-white">{item.stockNumber}</td>
                    <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-400">{item.inboundOrderNumber}</td>
                    <td className="px-3 py-4 text-sm text-gray-300 break-words">{item.description}</td>
                    <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-300 font-bold">{item.quantity}</td>
                    <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-400">{item.location}</td>
                    ...
                    <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-400">{formatDate(item.inboundDate)}</td>
                    <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-400">{item.inboundReference}</td>
                    <td className="px-3 py-4 whitespace-nowrap text-right text-sm font-medium">
                      ...
                      {view === 'active' && ( // Only show delete button for active inventory
                ...
                {(!Array.isArray(filteredInventory) || filteredInventory.length === 0) && !loading && (
                  <tr><td colSpan="9" className="px-3 py-4 text-center text-gray-400">{searchTerm || customerFilter ? "No pallets found matching your filters." : "No pallets in stock. Start by adding one!"}</td></tr>
                )}
              </tbody>
            </table>
          </div>
        </>
      <InventoryForm isOpen={isFormModalOpen} onClose={handleCancelForm} itemToEdit={itemToEdit} onSave={handleSaveAndCloseForm} />
