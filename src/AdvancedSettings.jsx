...
const AdvancedSettings = ({ user, onOpenScheduleSettings, onOpenRolesSettings }) => {
  ...
  const handleConfirmImport = async (mappings) => {
    if (!csvFile) return; // isImporting was removed, this check is sufficient
    setIsImporting(true);
    setImportProgress(0);
        ...
        const itemsToProcess = results.map(row => {
          ...
          const newItem = {
            ...
            inboundDate: (() => {
              ...
              if (dateValue) { // dateValue can be a Date object (from XLSX) or a string
                if (dateValue instanceof Date && !isNaN(dateValue.getTime())) {
                  // Handle native Excel dates - extract local YYYY-MM-DD
            ...
            originalTimeBooked: (() => {
              ...
              if (timeValue) {
                if (timeValue instanceof Date) {
                  // This is the key fix. We get the UTC hours and minutes from the date object
                  // that Excel provides. This avoids local timezone conversion issues.
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <div className="mt-4 p-4 bg-gray-900/50 border border-gray-700 rounded-lg space-y-3">
            ...
            {/* "Roles & Permissions" button removed as the feature was deleted */}
          </div>
        </div>
        ...
        {/* --- Danger Zone --- */}
        {/* Danger Zone is now always visible */}
