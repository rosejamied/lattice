    ...
    db.all("SELECT id, name FROM customers", [], (err, existingCustomers) => {
        ...
        items.forEach(item => {
            if (item.originalBookingReference || item.originalOrderDate) { // Only group items that have outbound details
                ...
                if (!ordersToCreate.has(orderKey)) {
                    ...
                    let finalTimestamp;
                    const datePart = item.originalOrderDate; // This is 'YYYY-MM-DD' from frontend
                    
                    if (datePart) {
                        if (item.originalTimeBooked) {
                            // Combine date and time, assume UTC for consistency
                            finalTimestamp = `${datePart}T${item.originalTimeBooked}:00`;
                        } else {
                            // Date only, set to noon UTC to avoid timezone shifts to the previous or next day.
                            finalTimestamp = `${datePart}T12:00:00`;
                        }
                    } else {
                        // Fallback if no date part, use current time
        ...
        db.serialize(() => {
            ...
            for (const [orderKey, orderData] of ordersToCreate.entries()) {
                ...
                if (orderData.status === 'Completed' || orderData.status === 'Dispatched') {
...
app.delete('/api/data/all-except-users', (req, res) => {
  ...
  const tablesToClear = [
    ...
    'customer_hauliers',
    'locations',
    // 'role_permissions', // Removed as per request
  ];
...
// GET /api/setup/status - Checks if any users exist in the database.
...
// POST /api/setup/create-admin - Creates the first admin user, only if no other users exist.
app.post('/api/setup/create-admin', (req, res) => {
  db.get("SELECT COUNT(*) as count FROM users", [], (err, row) => {
    if (err) {
      return res.status(500).json({ message: "Failed to check user count." });
    }
    if (row.count > 0) {
      return res.status(403).json({ message: "Setup has already been completed. Cannot create another initial admin." });
    }

    // If no users exist, proceed to create the first admin
    const { username, firstName, lastName, jobTitle, password, permissions } = req.body;
    if (!username || !password || !firstName || !lastName) {
      return res.status(400).json({ message: "Missing required fields for admin creation." });
    }

    const newUserId = `user_${Date.now()}`;
    // Use the same password hashing method as the main user creation route
    const salt = crypto.randomBytes(16).toString('hex');
    const passwordHash = crypto.pbkdf2Sync(password, salt, 1000, 64, 'sha512').toString('hex');
    const storedPassword = `${salt}:${passwordHash}`;

    const sql = `INSERT INTO users (id, username, firstName, lastName, role, jobTitle, passwordHash, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
    const role = 'Manager'; // The first user should always be a Manager.

    db.run(sql, [newUserId, username, firstName, lastName, role, jobTitle || 'Manager', storedPassword, new Date().toISOString()], function(err) {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
      // Don't send password hash back
      const userPayload = { id: newUserId, username, firstName, lastName, role, jobTitle };
      res.status(201).json({ message: "Admin user created successfully.", user: userPayload });
    });
  });
});
